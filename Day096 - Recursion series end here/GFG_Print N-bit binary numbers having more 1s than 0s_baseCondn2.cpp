...
public:	
	...
	void find(vector<string> &ans, string temp, int ones, int zeros, int n){
	    ...
	    temp.push_back('1');
	    find(ans, temp, ones+1, zeros, n-1);//icrease the count of 1  and generate numbers with two choices again 0 and 1
	    temp.pop_back();
	    ...
	    if(ones>zeros){
	        ...
	        temp.push_back('0');
	        find(ans, temp, ones, zeros+1, n-1);//increase the count of 0s and generate numbers with two choices again 0 and 1
	        temp.pop_back();
	    }
	}
};
